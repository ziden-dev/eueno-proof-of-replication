...
use bellperson::groth16::{
    ...
    prepare_verifying_key, verify_proof, verify_proofs_batch, Proof, prepare_single_verifying_key, verify_proof_single_thread, Parameters,
};
use std::iter;
...
fn test_mimc_single_thread() {
    ...
    for _ in 0..SAMPLES {
        ...
        let proof = Proof::read(&proof_vec[..]).unwrap();
        // Check the proof
        assert!(verify_proof_single_thread(&pvk, &proof, &[image]).unwrap());
        total_verifying += start.elapsed();
        proofs.push(proof);
        images.push(vec![image]);
    }
